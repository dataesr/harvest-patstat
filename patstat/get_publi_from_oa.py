...
def get_info_publi():
    ...
    col_url = ["id", "orcid", "pdf_url", "id_source", "id_ins", "ror_ins"]

    for col in col_url:
        oa2.loc[oa2[col].notna(), col] = oa2.loc[oa2[col].notna(), col].apply(unquote)

    columns = ["doi_clean", "display_name", "display_name_title", "language", "type", "volume", "issue",
               "raw_source_name",
               "issn_l", "host_organization_name", "display_name_ins", "country_code_ins", "type_ins", "idref_ins"]
