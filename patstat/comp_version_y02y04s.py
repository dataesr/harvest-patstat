...
# Ce programme permet de créer le fichier sur "TECHNOLOGIES OR APPLICATIONS FOR MITIGATION OR ADAPTATION AGAINST CLIMATE
# CHANGE" (classification Y02 CPC) et "SYSTEMS INTEGRATING TECHNOLOGIES RELATED TO POWER NETWORK OPERATION,
# COMMUNICATION OR INFORMATION TECHNOLOGIES FOR IMPROVING THE ELECTRICAL POWER GENERATION, TRANSMISSION, DISTRIBUTION,
# MANAGEMENT OR USAGE, i.e. SMART GRIDS" (classification Y04S CPC) pour au format JSON qui va alimenter ScanR
...
def get_json_env():
    ...
    # filtre sur les codes CPC portant sur les brevets consacrés aux technologies d'atténuation ou d'adaptation
    # aux changements climatiques

    fam_techno = fam_techno.loc[
        fam_techno["code"].isin(["Y02A", "Y02B", "Y02C", "Y02D", "Y02E", "Y02P", "Y02T", "Y02W", "Y04S"])]
    fam_techno = fam_techno.sort_values("code")

    patent = patent.loc[patent["docdb_family_id"].isin(fam_techno["docdb_family_id"])]

    fam = fam.loc[fam["docdb_family_id"].isin(fam_techno["docdb_family_id"])]

    part = part.loc[part["docdb_family_id"].isin(fam_techno["docdb_family_id"])]

    role = role.loc[role["key_appln_nr_person"].isin(part["key_appln_nr_person"])]
    ...
    fam_final.to_json("fam_final_json_env.jsonl", orient="records", lines=True)
    swift.upload_object('patstat', 'fam_final_json_env.jsonl')
